<html>
<head>
<title>Covid-19 Tracker Indonesia</title>
...
?>
  <tr>
      <td>              <?php echo  $tampil[0]['name'];                    ?></td>
      <td id='kuning'>  <?php echo  $tampil[0]['positif'];                 ?></td>
      <td id='hijau'>   <?php echo  $tampil[0]['sembuh'];                  ?></td>
      <td id='merah'>   <?php echo  $tampil[0]['meninggal'];               ?></td>
  </tr>
 </table>
<h3> Data Kasus Covid-19 Secara Keseluruhan </h1>
<?php function curl1($url) { $ch = curl_init(); curl_setopt($ch, CURLOPT_URL, $url); curl_setopt($ch, CURLOPT_RETURNTRANSFER, true); curl_setopt($ch, CURLOPT_CUSTOMREQUEST, "GET"); $headers   = array(); $headers[] = 'Authority: api.kawalcorona.com'; $headers[] = 'Pragma: no-cache'; $headers[] = 'Cache-Control: no-cache'; $headers[] = 'Upgrade-Insecure-Requests: 1'; $headers[] = 'User-Agent: Mozilla/5.0 (Windows NT 6.1; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/76.0.3809.132 Safari/537.36'; $headers[] = 'Sec-Fetch-Mode: navigate'; $headers[] = 'Sec-Fetch-User: ?1'; $headers[] = 'Accept: text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,/;q=0.8,application/signed-exchange;v=b3'; $headers[] = 'Sec-Fetch-Site: none'; $headers[] = 'Accept-Language: en-US,en;q=0.9,id;q=0.8'; curl_setopt($ch, CURLOPT_HTTPHEADER, $headers); $exec = curl_exec($ch); return $exec; curl_close($ch); }
    ################## POSITIF CORONA #########################
    $apipositif    = "https://api.kawalcorona.com/positif/";
    $positif  = curl1($apipositif);
    $nampilpositif   = json_decode($positif, true);
	################## SEMBUH CORONA #########################
	$apisembuh    = "https://api.kawalcorona.com/sembuh/";
    $sembuh  = curl1($apisembuh);
    $nampilsembuh   = json_decode($sembuh, true);
	################ MENINGGAL CORONA #########################
	$apimeninggal    = "https://api.kawalcorona.com/meninggal/";
    $meninggal  = curl1($apimeninggal);
    $nampilmeninggal   = json_decode($meninggal, true);
    echo "<table>
	      <tr>
	      <th id='kuning'>Positif       </th>
	      <th id='hijau'>Sembuh         </th> 
...
?>
  <tr>
      <td id='kuning'>  <?php echo  $nampilpositif['value'];          ?></td>
      <td id='hijau'>   <?php echo  $nampilsembuh['value'];           ?></td>
      <td id='merah'>   <?php echo  $nampilmeninggal['value'];        ?></td>
  </tr>
 </table>
<br/>
